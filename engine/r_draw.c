...
void r_ExecuteImmediateCmdBuffer(struct r_i_cmd_buffer_t *cmd_buffer)
{
...
//                break;
//
//                case R_I_CMD_APPLY_DRAW_STATE:
//                {
//                    struct r_i_draw_state_t *draw_state = cmd->data;
//
//                    if(draw_state->shader)
//                    {
//                        struct r_i_shader_t *shader = draw_state->shader;
//
//                        if(shader->shader)
//                        {
                ...
                case R_I_CMD_DRAW:
                {
                    ...
                    else
                    {
                        for(uint32_t range_index = 0; range_index < draw_list->range_count; range_index++)
                        {
                            ...
                            glDrawArrays(draw_list->mode, range->start, range->count);
                        }
                    }
                }
                break;
...
//    glDisable(GL_STENCIL_TEST);
    ...
    r_ExecuteImmediateCmdBuffer(&r_immediate_cmd_buffer);

//    for(uint32_t cmd_index = 0; cmd_index < r_immediate_cmd_buffer.cmds.cursor; cmd_index++)
//    {
//        struct r_i_cmd_t *cmd
//    }
//
...
//    r_BindFramebuffer(r_main_framebuffer);
//    r_BindShader(r_blend_ui_debug_shader);
//    r_BindTexture(r_main_framebuffer->depth_attachment, GL_TEXTURE0);
//    r_SetDefaultUniformI(R_UNIFORM_TEX0, 0);
//    r_BindTexture(r_debug_framebuffer->depth_attachment, GL_TEXTURE1);
//    r_SetDefaultUniformI(R_UNIFORM_TEX1, 1);
//    r_BindTexture(r_debug_framebuffer->color_attachments[0], GL_TEXTURE2);
//    r_SetDefaultUniformI(R_UNIFORM_TEX2, 2);
//    r_BindTexture(r_ui_framebuffer->color_attachments[0], GL_TEXTURE3);
//    r_SetDefaultUniformI(R_UNIFORM_TEX3, 3);
//    glDisable(GL_DEPTH_TEST);
//    glDisable(GL_SCISSOR_TEST);
//    glDisable(GL_STENCIL_TEST);
//    glEnable(GL_BLEND);
